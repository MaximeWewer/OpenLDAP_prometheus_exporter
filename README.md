...
Peers are discovered automatically from the local `olcSyncrepl` `provider=` URIs (requires config-tree read access), and can be supplemented/overridden with `OPENLDAP_REPLICATION_PEERS`. Discovery re-runs on **every poll tick**, so peers added to or removed from `cn=config` are picked up live (a new peer's client is created, a removed peer's client is closed and its metric series pruned) — no restart needed for auto-discovered peers. A peer is only removed when the `cn=config` read succeeds, so a transient read failure never tears down known peers. URIs are normalized (scheme/host case, default port, whitespace) before de-duplication; a short hostname, FQDN and IP for the same server are **not** merged (that would need DNS resolution), so use consistent naming to avoid double-counting.

For each peer it exposes `openldap_replication_peer_up`, `openldap_replication_peer_csn_timestamp` and `openldap_replication_peer_lag_seconds` (= reference CSN − peer CSN, where the reference is the max of the local node and all peers for that `server_id`). In a full-mesh N-way multi-master cluster every other node is auto-discovered; in a non-mesh topology (chain/star), list the nodes this one does not pull from in `OPENLDAP_REPLICATION_PEERS`.
